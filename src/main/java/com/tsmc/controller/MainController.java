...
public class MainController {
    ...
    public ModelAndView createPrize(@ModelAttribute MainForWinnerEntity mainForWinnerEntity, Model model,
    ...
    public ModelAndView randomWinner(@ModelAttribute PrizePeopleModel prizePeopleModel, Model model,
            HttpServletRequest request) {
        ModelAndView modelAndView = new ModelAndView("views/findPrize/findPrize");
        if (Integer.parseInt(prizePeopleModel.getPrizeName()) < 2
                || prizePeopleModel.getCurrentPeople().trim().equals(prizePeopleModel.getPrizePeople().trim())) {
    ...
    public ModelAndView findPrize(@ModelAttribute FindNowPrizeModel findNowPrizeModel, Model model,
            HttpServletRequest request) {
        ModelAndView modelAndView = new ModelAndView("views/findPrize/findPrize");
